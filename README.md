...
```
src/
├── index.ts                  # boot, provider connections, graceful shutdown
├── config/
│   └── env.ts                # environment reading and validation (zod)
├── core/
│   ├── tool-response.ts      # the ToolResponse contract and its factories
│   ├── errors.ts             # ToolError and generic classification
│   ├── tool-name.ts          # {GATEWAY}_{PROVIDER}_{TOOL}
│   ├── tool-registrar.ts     # applies name + envelope to every tool
│   ├── serialization.ts      # safe JSON (BigInt, Buffer, cycles, ...)
│   └── logger.ts             # structured JSON logging
├── providers/
│   ├── index.ts              # Provider contract + BaseProvider/ConnectedProvider/ProviderErrorMapper
...
├── server/
│   ├── mcp-server.ts         # builds the McpServer and registers the tools
│   └── http.ts               # Express: /mcp, /health, /
├── testing/
│   └── fake-mcp-server.ts    # McpServer double used by the tests
└── tools/
...
Each unit test sits next to the file it exercises, with the `.unit.spec.ts`
suffix:
...
That way the test shows up next to the code in the editor and follows the file
when it moves. `jest.config.js` looks for `**/*.unit.spec.ts` inside
`src/`, and `tsconfig.build.json` excludes those files along with
`src/testing/`, so no test reaches `dist/` or the Docker image.

The suffix leaves room for other levels later (`.int.spec.ts`, `.e2e.spec.ts`)
to run with their own configuration, without touching these.

Two central pieces hold up the project's guarantees:

- **`ToolRegistrar`** is the only way to register a tool. It applies the naming
  pattern, advertises the `outputSchema` and wraps the handler in a try/catch
  that turns any exception into the envelope. No tool can escape the contract.
- **`Provider`** is the interface every backend implements (`connect`,
  `disconnect`, `checkHealth`, `registerTools`), which keeps the status tool and
  `/health` working the same way for all of them, present and future.
  `BaseProvider` and `ConnectedProvider` (in `src/providers/index.ts`) already
  deliver that interface ready-made: identity, tagged logger, connection
  lifecycle with a single open under concurrency, and the health check flow. The
  subclass fills in only what belongs to the backend — `connectionUrl`,
  `openConnection`, `closeConnection`, `probe` and `defineTools`.

---

## Adding a new provider

Each backend is a vertical slice: **one file** in `src/providers/` with
everything that belongs to it (provider, error mapper and whatever else is
specific), plus the `.unit.spec.ts` next to it.

1. Create `src/providers/<Name>Provider.ts` with a class extending
   `ConnectedProvider<TConnection>` (or `BaseProvider`, if there is no live
   connection), implementing `connectionUrl`, `openConnection`,
   `closeConnection`, `probe` and `defineTools`.
2. In the same file, create `<Name>ErrorMapper extends ProviderErrorMapper`
   mapping the driver errors onto the four categories — the cascade
   (classification → network failure → business error) already comes from the
   base class.
3. Declare the tools in `defineTools(registrar)` with
   `this.tool(registrar, {...})` — the provider segment, the full name and the
   envelope come for free.
4. Add the connection URL to `src/config/env.ts` and return it from
   `connectionUrl`, so the provider stays optional.
5. Instantiate the provider in the list in `src/index.ts`.

> `src/providers/index.ts` holds only the contract and the base classes; it does
> **not** re-export the concrete providers, because that would create a runtime
> cycle with the subclasses. Import each provider from its own file.
